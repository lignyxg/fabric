...
var (
	...
	//TODO this value is specified no where, need to confirm next
...
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey crypto.PublicKey) (err error) {
	...
	for _, details := range signatureAlgorithmDetails {
		if details.algo == algo {
			//if algo != SM2WithSM3 { // need handle separately if algo is SM2WithSM3
	...
	default:
		// this lib is used as vendor, the golang env may not support SM (crypto/crypto.go)
		// so the 'SM2WithSM3' need handle separately
		if algo == SM2WithSM3 {
			h = sm3.New()
		} else {
